...
- Latarnia: publiczne endpointy dostępności KSeF (`client.lighthouse`, `uv run ksef lighthouse ...`).
...
```bash
uv sync --extra xml --extra qr --extra cli
```
...
Zainstaluj wszystkie zależności (produkcyjne + deweloperskie):
...
```bash
# Omit password for secure prompt input (recommended)
...
Wymaga dodatku `xml`: `uv sync --extra xml`.
...
```bash
uv run pytest --cov=ksef_client --cov-report=term-missing --cov-fail-under=100
```
...
- utrzymaj jakość: `uv run pytest`, `uv run pytest --cov=ksef_client --cov-fail-under=100`,
